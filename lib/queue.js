...
queueTasks.doProcessImport = async function (jobId, input) {
  ...
  await cqa('match (a:Person)-[:SOURCE]->(b:ImportRecord)-[:FILE]->(c:ImportFile {filename:{filename}})-[:ATTRIBUTES]->(d:Attribute) create (e:PersonAttribute {value:b[d.name]})-[:ATTRIBUTE_OF {current:true, updated: timestamp()}]->(a) create (e)-[:COLLECTED_ON]->(c) create (e)-[:ATTRIBUTE_TYPE]->(d)', {filename: filename});
