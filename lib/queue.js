...
queueTasks.doAddAddress = async function (jobId, input) {
  let start = new Date().getTime();
  await cqa('match (a:ReferenceNode {name:"spatial_root"}) with collect(a) as lock call apoc.lock.nodes(lock) match (a:Address {id:{addressId}}) call spatial.addNode("address", a) yield node return node', input);
