...
export default class queue {
  ...
  constructor(db) {
    ...
    this.queue.on('doTask', async function (id) {
      ...
      try {
        let job = await this.db.query('match (a:QueueTask {id:{id}}) return a', {id: id});
        ...
        // mark job as success
        await this.db.query('match (a:QueueTask {id:{id}}) set a.active = false, a.completed = timestamp(), a.success = true, a.error = null', {id: id});
      } catch (e) {
        console.warn("Caught exception while executing task: "+task);
        console.warn(e);
        // mark job as failed
        await this.db.query('match (a:QueueTask {id:{id}}) set a.active = false, a.completed = timestamp(), a.success = false, a.error = {error}', {
          id: id,
          error: e.toString(),
        });
        error = true;
    ...
    this.queue.on('checkQueue', async function () {
      ...
      try {
        let ref = await this.db.query('match (a:QueueTask {active: true}) return count(a)');
        ...
        let job = await this.db.query('match (a:QueueTask) where a.active = false and not exists(a.started) with a limit 1 set a.active = true, a.started = timestamp() return a');
        if (!job.data[0]) {
        ...
        if ((running+1) < concurrency) this.queue.emit('checkQueue');
  ...
  async doGeocode(data) {
    ...
    try {
      ...
      // update database
      await this.db.query('unwind {data} as r match (a:Address {id:r.id}) set a.street = r.street, a.city = r.city, a.state = r.state, a.zip = r.zip, a.position = point({longitude: toFloat(r.longitude), latitude: toFloat(r.latitude)})', {data: data});

      // update ids
      await this.db.query('unwind {data} as r match (a:Address {id:r.id}) set a.id = apoc.util.md5([toLower(a.street), toLower(a.city), toLower(a.state), substring(a.zip,0,5)])', {data: data});
