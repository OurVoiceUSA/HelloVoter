...
export async function queueTask(task, pattern, input) {
  ...
  try {
    ...
    job = await cqa('match (a:QueueTask {active: true}) with count(a) as jobs call apoc.do.when(jobs < '+concurrency+', "create (a:QueueTask {id: randomUUID(), created: timestamp(), started: timestamp(), task: task, input: input, active: true}) return a", "create (a:QueueTask {id: randomUUID(), created: timestamp(), task: task, input: input, active: false}) return a", {task: {task}, input: {input}}) yield value match (a:QueueTask {id:value.a.id}) match (b:'+pattern+') merge (b)-[:PROCESSED_BY]->(a) return a', args);
  } catch (e) {
...
queueTasks.doAddAddress = async function (jobId, input) {
  let start = new Date().getTime();
  await cqa('match (a:ReferenceNode {name:"spatial_root"}) with collect(a) as lock call apoc.lock.nodes(lock) match (a:Address {id:{addressId}}) call spatial.addNode("address", a) yield node call spatial.intersects("turf", a.position) yield node as turf merge (a)-[:WITHIN]->(turf) return turf', input);
...
queueTasks.doProcessImport = async function (jobId, input) {
  ...
  // turf count
  let tref = await cqa('match (t:Turf) return count(t)');

  // if turf count is zero, don't bother with the turf indexing
  if (tref.data[0] !== 0) {
    // create a temporary point layer to do turf indexing
    await cqa('call spatial.addLayerWithEncoder({filename}, "NativePointEncoder", "position")', {filename: filename});
  ...
  if (ov_config.purge_import_records) await cqa('match (f:ImportFile {filename:{filename}}) call apoc.periodic.commit("match (:ImportFile {filename:\\""+f.filename+"\\"})<-[:FILE]-(r:ImportRecord) with r limit 1000 detach delete r return count(r)", {}) yield executions return executions', {filename: filename});
}
...
async function doGeocode(data) {
  ...
  try {
    ...
    // update database
    await cqa('unwind {data} as r match (a:Address {id:r.id}) set a.street = r.street, a.city = r.city, a.state = r.state, a.zip = r.zip, a.position = point({longitude: toFloat(r.longitude), latitude: toFloat(r.latitude)})', {data: data});
