...
async function analyticsList(req, res) {
  return cqdo(req, res, 'match (:Address)<-[:AT*0..1]-()<-[:RESIDENCE]-(:Person)<-[:ATTRIBUTE_OF]-(b:PersonAttribute)-[:ATTRIBUTE_TYPE]->(:Attribute {name:"Party Affiliation"}) with distinct(b.value) as dist, count(b.value) as count return dist, count', {}, true);
}
