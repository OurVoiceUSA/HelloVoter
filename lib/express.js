...
async function hello(req, res) {
  ...
  try {
    ...
    let dinfo_str = ['ApplicationName', 'Brand', 'BuildNumber', 'BundleId', 'Carrier', 'DeviceCountr', 'DeviceId', 'DeviceLocale', 'DeviceName', 'FontScale', 'FreeDiskStorage', 'Manufacturer', 'Model', 'ReadableVersion', 'SystemName', 'SystemVersion', 'Timezone', 'TotalDiskCapacity', 'TotalMemory', 'UniqueID', 'UserAgent', 'Version', '24Hour', 'Emulator', 'Tablet', 'hasNotch', 'Landscape', 'DeviceType'].map(d => d+':{'+d+'}').join(',');

    await cqa('match (a:Volunteer {id:{id}}) merge (b:Device {UniqueID:{UniqueID}}) on create set b += {created: timestamp(), updates: timestamp(), '+dinfo_str+'} on update set b += {updated: timestamp(), '+dinfo_str+'} merge (a)<-[:USED_BY]-(b)', {id: req.user.id, UniqueID: req.body.dinfo.UniqueID});
...
`+((!req.user.admin&&req.query.formId)?'where not (person)<-[:INTERACTION_PERSON]-(:Interaction)-[:INTERACTION_FORM]->(:Form {id:{formId}})':'')+`
...
`+((!req.user.admin&&req.query.formId)?'where size(people) > 0 or u is null':'')+`
    with a, u{.*, people: people} as unit
...
`+((!req.user.admin&&req.query.formId)?'where size(people) > 0 or size(units) > 0':'')+`
  return collect({address: a{longitude:ap.x,latitude:ap.y,.street,.city,.state,.zip,.updated}, units: units, people: people}) as data
      `, req.query);
...
async function dataAdd(req, res) {
  ...
  if (!req.body.status) return _400(res, "Invalid value to parameter 'status'.");
  if (typeof req.body.attrs !== 'object') return _400(res, "Invalid value to parameter 'dinfo'.");
  ...
  // TODO: person required if there's data, and status is "home"
  ...
  match (p:Person {id:{personId}})-[:RESIDENCE]->`+(req.body.unit?'(u:Unit {name:{unit}})-[:AT]->':'')+`(a:Address {id:{addressId}})-[:WITHIN]->(t)
  ...
  merge (i)-[:INTERACTION_DEVICE]->(d)
  merge (i)-[:INTERACTION_VOLUNTEER]->(v)
  merge (i)-[:INTERACTION_AT]->(`+(req.body.unit?'u':'a')+`)
  merge (i)-[:INTERACTION_FORM]->(f)
  merge (i)-[:INTERACTION_PERSON]->(p)
    set
      `+(req.body.unit?'u':'a')+`.status = {status}
      `+(req.body.unit?'u':'a')+`.updated = timestamp()
    with i, p
