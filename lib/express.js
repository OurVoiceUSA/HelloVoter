...
async function volunteerAssignments(user) {
  ...
  try {
    ...
    if (user.autoturf && user.location) {
      obj.turfs.push({id: 'auto', name: 'auto', geometry: circleToPolygon([user.location.x,user.location.y],1000)});
    }
...
async function volunteerUpdate(req, res) {
  ...
  try {
    await cqa('match (a:Volunteer {id:{id}}) set a.locationstr={address}, a.location = point({longitude: {lng}, latitude: {lat}}), a.wkt={wkt}', req.body);
  } catch (e) {
    return _500(res, e);
  }
...
async function dataAdd(req, res) {
  ...
  `+(req.body.personId?'match (p:Person {id:{personId}})-[:RESIDENCE]->':'')+(req.body.unit?'(u:Unit {name:{unit}})-[:AT]->':'')+`(a:Address {id:{addressId}})-[:WITHIN]->(t)
