...
import { public_key, jwt_iss } from './pubkey';
...
// TODO: alphanumeric non-shell characters
...
async function volunteerCanSee(ida, idb) {
  ...
  if (await sameTeam(ida, idb)) return true;
  if (await onMyTurf(ida, idb)) return true;
  return false;
}
...
async function onMyTurf(ida, idb) {
  ...
  try {
    ...
    let ref = await db.query('match (v:Volunteer {id:{idb}}) where exists(v.location) call spatial.intersects("turf", v.location) yield node match (:Volunteer {id:{ida}})-[:MEMBERS {leader:true}]-(:Team)-[:ASSIGNED]-(node) return count(v)', {ida: ida, idb: idb});
...
async function sameTeam(ida, idb) {
  try {
    let ref = await db.query('match (a:Volunteer {id:{ida}})-[:MEMBERS]-(:Team)-[:MEMBERS]-(b:Volunteer {id:{idb}}) return b', {ida: ida, idb: idb});
...
async function volunteerAssignments(user) {
  ...
  try {
    ...
    if (ov_config.autoenroll_formid) {
      let b = await db.query('match (b:Form {id:{formId}}) return b{.*,direct:true}', {formId: ov_config.autoenroll_formid});
  ...
  if (typeof req.body.dinfo === 'object') {
    try {
      ...
      if (ov_config.debug) {
        ...
        let ref = await db.query('match (a:Volunteer {admin:true}) return count(a)');
        if (ref.data[0] === 0) {
          await db.query('match (a:Volunteer {id:{id}}) set a.admin=true', req.user);
          req.user.admin = true;
          ass.admin = true;
        }
      }
      ...
      await db.query('match (v:Volunteer {id:{id}}) merge (b:Device {UniqueID:{UniqueID}}) on create set b += {created: timestamp(), updated: timestamp(), '+dinfo_str+'} on match set b += {updated: timestamp(), '+dinfo_str+'} merge (v)<-[:USED_BY]-(b)', args);

      if (isNaN(args.lng) || isNaN(args.lat)) return _400(res, "Invalid value to parameters 'longitude' and 'latitude'.");

      await db.query('match (v:Volunteer {id:{id}}) set v.position = point({longitude: {lng}, latitude: {lat}})', args);
      ...
      if (!req.user.location) {
        try {
          ...
          await db.query('match (v:Volunteer {id:{id}}) set v.location = point({longitude: {lng}, latitude: {lat}}), v.locationstr = {str}', args);
        } catch (e) {
          console.warn(e);
        }
      }
...
async function dashboard(req, res) {
  try {
    let nv = await db.version();
    if (req.user.admin === true) return res.json({
      volunteers: (await db.query('match (a:Volunteer) return count(a)')).data[0],
      teams: (await db.query('match (a:Team) return count(a)')).data[0],
      turfs: (await db.query('match (a:Turf) return count(a)')).data[0],
      attributes: (await db.query('match (a:Attribute) return count(a)')).data[0],
      forms: (await db.query('match (a:Form) return count(a)')).data[0],
      addresses: (await db.query('match (a:Address) return count(a)')).data[0],
...
async function _volunteersFromCypher(query, args) {
  ...
  let ref = await db.query(query, args)
...
async function volunteerUpdate(req, res) {
  ...
  try {
    await db.query('match (a:Volunteer {id:{id}}) set a.locationstr={address}, a.location = point({longitude: {lng}, latitude: {lat}})', req.body);
  } catch (e) {
    return _500(res, e);
  }
...
async function volunteerLock(req, res) {
  ...
  try {
    let ref = await db.query("match (a:Volunteer {id:{id}}) return a", req.body);
...
async function teamCreate(req, res) {
  ...
  await db.query('match (a:Volunteer {id:{author_id}}) create (b:Team {id:{teamId}, created: timestamp(), name:{name}})-[:CREATOR]->(a)', req.body);

  return res.json({teamId: req.body.teamId});
}
...
async function volunteerIsLeader(id, teamId) {
  try {
    let ref = await db.query('match (:Volunteer {id:{id}})-[:MEMBERS {leader:true}]-(a:Team {id:{teamId}}) return a', {id: id, teamId: teamId})
    if (ref.data.length > 0) return true;
  } catch (e) {
    console.warn(e);
  }
...
function turfListByposition(req, res) {
...
async function turfCreate(req, res) {
  ...
  try {
    // create Turf
    await db.query('match (a:Volunteer {id:{author_id}}) create (b:Turf {id:{turfId}, created: timestamp(), name:{name}, geometry: {geometry}, wkt:{wkt}})-[:AUTHOR]->(a) WITH collect(b) AS t CALL spatial.addNodes(\'turf\', t) YIELD count return count', req.body);
  } catch(e) {
    return _500(res, e);
  }

  let job = await qq.queueTask('doTurfIndexing', 'Turf {id:{turfId}}', {turfId: req.body.turfId});

  job.turfId = req.body.turfId;
...
async function turfDelete(req, res) {
  ...
  try {
    await db.query('match (a:Turf {id:{turfId}}) detach delete a', req.body);
  } catch(e) {
    return _500(res, e);
  }
...
async function turfAssignedVolunteerAdd(req, res) {
  ...
  if (!req.body.override) {
    try {
      ...
      ret = await db.query('match (a:Volunteer {id:{vId}}) return a', req.body);
      let c = ret.data[0];

      ret = await db.query('match (a:Turf {id:{turfId}}) return a', req.body);
      let t = ret.data[0];
...
async function formGet(req, res) {
  ...
  try {
    let a = await db.query('match (a:Form {id:{formId}})-[:AUTHOR]-(b:Volunteer) return a,b', req.query);
    form = a.data[0][0];
    ...
    form.attributes = [];
    let b = await db.query('match (a:Attribute)-[c:COMPILED_ON]->(b:Form {id:{formId}}) return a{.*, readonly: c.readonly}', req.query);
    for (let o in order) {
      let id = order[o];
      for (let i in b.data) {
    ...
    let c = await db.query('match (t:Turf) where t.id in {turfIds} return t.geometry', {turfIds: idFromArrObj(ass.turfs)});
...
async function formCreate(req, res) {
  ...
  try {
    ...
    ref = await db.query('match (a:Volunteer {id:{author_id}}) create (b:Form {id: randomUUID(), created: timestamp(), updated: timestamp(), name:{name}, attributes:{attributes}})-[:AUTHOR]->(a) with b unwind {attributes} as attr match (a:Attribute {id:attr}) merge (a)-[:COMPILED_ON]->(b) return b.id', req.body);
  } catch (e) {
    return _500(res, e);
  }
...
async function importBegin(req, res) {
  ...
  try {
    let ref = await db.query('match (a:ImportFile {filename:{filename}}) where a.submitted is not null return count(a)', req.body);
    if (ref.data[0] !== 0) return _403(res, "Import File already exists.");

    // attributes property stores which order they come in as
    await db.query('match (a:Volunteer {id:{id}}) merge (b:ImportFile {filename:{filename}}) on create set b += {id: randomUUID(), created: timestamp(), attributes: {attributes}} merge (b)-[:IMPORTED_BY]->(a) with b unwind {attributes} as attr match (a:Attribute {name:attr}) merge (b)-[:ATTRIBUTES]->(a)', req.body);
  } catch (e) {
    return _500(res, e);
  }
...
async function importAdd(req, res) {
  ...
  try {
    ...
    let attrq = "";
    let ref = await db.query('match (a:ImportFile {filename:{filename}}) return a.attributes', req.body);
...
async function importEnd(req, res) {
  ...
  try {
    let ref = await db.query('match (a:ImportFile {filename:{filename}}) where a.submitted is null set a.submitted = timestamp() return count(a)', req.body);
    if (ref.data[0] !== 1) return _403(res, "Import File already submitted for processing.");
  } catch (e) {
    return _500(res, e);
  }

  let job = await qq.queueTask('doProcessImport', 'ImportFile {filename:{filename}}', {filename: req.body.filename});
...
async function addressGetByposition(req, res) {
...
async function addressAddLocation(req, res) {
...
async function addressAddUnit(req, res) {
...
async function addressAdd(req, res) {
  ...
  let ref = await db.query(`match (v:Volunteer {id:{id}})<-[:USED_BY]-(d:Device {UniqueID:{deviceId}}), (f:Form {id:{formId}})
  ...
  if (req.body.addressId && req.body.unit) {
    ...
    await qq.queueTask('doAddAddress', 'Address {id:{addressId}}', req.body);
  }
...
async function peopleGetByaddress(req, res) {
  if (!req.query.aId) return _400(res, "Invalid value to parameter 'aId'.");
...
*/
  ...
  await volunteerAssignments(req.user);
  ...
  optional match (attr:Attribute)<-[:ATTRIBUTE_TYPE]-(pattr:PersonAttribute)-[:ATTRIBUTE_OF {current:true}]->(person)
    ...
    with a, units, a.position as ap, collect(person{.*, attrs: attrs}) as people `;
      ...
      ref = await db.query(q, req.query);
...
async function peopleVisitUpdate(req, res) {
  ...
  try {
    ...
    if (req.addnewperson) {
      ...
      let ref = await db.query('match (p:Person {id:{personId}}) return count (p)', req.body);

      if (ref.data[0] > 0) return _403(res, "Person already exists.");

      await db.query('match (a:Address {id:{addressId}})'+(req.body.unit?'<-[:AT]-(u:Unit {name:{unit}})':'')+' create (p:Person {id:{personId}}) create (p)-[r:RESIDENCE {current:true}]->'+(req.body.unit?'(u)':'(a)'), req.body);
    }
...
async function volunteerVisitHistory(req, res) {
  let ref = {};
...
export function doExpressInit(log, dbc, qqc) {
  db = dbc;
  qq = qqc;
  ...
  app.use(async function (req, res, next) {
    ...
    try {
      ...
      let a = await db.query('merge (a:Volunteer {id:{id}}) on match set a += {last_seen: timestamp(), name:{name}, email:{email}, avatar:{avatar}} on create set a += {created: timestamp(), last_seen: timestamp(), name:{name}, email:{email}, avatar:{avatar}} return a', u);
      if (a.data.length === 1) {
        req.user = a.data[0];
  ...
  app.get(base_uri+'/volunteer/visit/history', volunteerVisitHistory);
  ...
  app.get(base_uri+'/turf/list/byposition', turfListByposition);
  ...
  app.get(base_uri+'/address/get/byposition', addressGetByposition);
  app.post(base_uri+'/address/add/location', addressAddLocation);
  app.post(base_uri+'/address/add/unit', addressAddUnit);
  app.get(base_uri+'/people/get/byposition', peopleGetByposition);
  app.get(base_uri+'/people/get/byaddress', peopleGetByaddress);
...
export function doExpressStartup(dbc, qqc) {
  const app = doExpressInit(true, dbc, qqc);
  ...
  if (ov_config.server_ssl_port && ov_config.server_ssl_key && ov_config.server_ssl_cert) {
    ...
    https.createServer(options, app).listen(ov_config.server_ssl_port);
  } else {
