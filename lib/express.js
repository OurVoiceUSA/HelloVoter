...
async function hello(req, res) {
  ...
  try {
    ...
    await cqa('match (a:Volunteer {id:{id}}) merge (b:Device {UniqueID:{UniqueID}}) on create set b += {created: timestamp(), updates: timestamp(), '+dinfo_str+'} on match set b += {updated: timestamp(), '+dinfo_str+'} merge (a)<-[:USED_BY]-(b)', req.body.dinfo);
...
`+((!req.user.admin&&req.query.formId)?'where size(people) > 0 or size(units) > 0':'')+`
  return collect({address: a{longitude:ap.x,latitude:ap.y,.id,.street,.city,.state,.zip,.updated}, units: units, people: people}) as data
      `, req.query);
