...
match (a:ImportFile {filename:{filename}})
  unwind {data} as r
create (b:ImportRecord {id:apoc.util.md5([r[0],r[1],r[2],r[3],r[4],r[5],r[6],r[7]]), pid:r[0], street:r[1], unit:r[2], city:r[3], state:r[4], zip:r[5], lng:r[6], lat:r[7]`+attrq+`})
merge (b)-[:FILE]->(a)`,
      req.body);
  } catch (e) {
    return _500(res, e);
...
async function addressGetByposition(req, res) {
  ...
  return collect({address: a{longitude:a.position.x,latitude:a.position.y,.id,.street,.city,.state,.zip,.updated}})
    `, req.query);
