...
async function analyticsList(req, res) {
  return cqdo(req, res, 'match (:Address)<-[:AT*0..1]-()<-[:RESIDENCE]-(a:Person) '+(req.query.include_null?'optional match':'match')+' (a)<-[:ATTRIBUTE_OF]-(b:PersonAttribute)-[:ATTRIBUTE_TYPE]->(:Attribute {name:"Party Affiliation"}) with distinct(b.value) as dist, count(*) as count return dist, count order by count desc', {}, true);
}
...
export function doExpressStartup() {
  ...
  app.get('/volunteer/v1/analytics/list', analyticsList);
