...
async function dataGet(req, res) {
  ...
  try {
    ...
    ref = await cqa(`
  match (a:Volunteer {id:{id}}) with a
  optional match (b:Turf)-[:ASSIGNED]->(:Team)-[:MEMBERS]->(a) with a, collect(b.id) as tt
  optional match (b:Turf)-[:ASSIGNED]->(a) with tt + collect(b.id) as turfIds
  ...
  optional match (person:Person)-[:RESIDENCE]->(unit:Unit)-[:AT]->(a) with a, collect(person{.*, unit:unit.name}) as upeople
  optional match (person:Person)-[:RESIDENCE]->(a) with a, upeople + collect(person{.*}) as people
  ...
  match (attr:Attribute)<-[:ATTRIBUTE_TYPE]-(pattr:PersonAttribute)-[:ATTRIBUTE_OF]->(:Person {id:person.id})
    ...
    with a, collect({person: person, attrs: attrs}) as people
  return a, people;
    `, req.query);
  } catch (e) {
    return _500(res, e);
