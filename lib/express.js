...
`+(req.query.formId?'optional match (u)<-[:VISIT_AT]-(v:Visit)-[:VISIT_FORM]->(:Form {id:{formId}}) where v.status in {visit_status} with a, u, people, collect(v) as visits, collect(v.status) as status where not 2 in status or status is null':'')+`
