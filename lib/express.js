...
async function volunteerAssignments(user) {
  ...
  try {
    ...
    if (user.autoturf && user.homelng && user.homelat) {
      obj.turfs.push({id: 'auto', name: 'auto', geometry: circleToPolygon([user.homelng,user.homelat],1000)});
    }
...
async function hello(req, res) {
  if (typeof req.body.dinfo !== 'object') return _400(res, "Invalid value to parameter 'dinfo'.");
  if (!req.body.dinfo.UniqueID) return _400(res, "Invalid value to parameter 'dinfo.UniqueID'.");
...
*/
  ...
  if (ass.ready) {
    // save their device info
    // TODO: save all the dinfo properties
    await cqa('match (a:Volunteer {id:{id}}) merge (b:Device {UniqueID:{UniqueID}}) on create set b += {created: timestamp()} merge (a)<-[:USED_BY]-(b)', {id: req.user.id, UniqueID: req.body.dinfo.UniqueID});
...
async function dataGet(req, res) {
  ...
  if (!req.user.admin) {
    ...
    req.query.limit = 10; // TODO: server setting for this?
  ...
  try {
    // TODO: does this user get data from the address index, or a walk list?
    // TODO: form attribute constraints to only show people this form needs to collect data from

    // return address, people ... and "unit" is a property of people
    ref = await cqa(`
  match (a:Volunteer {id:{id}})
    with a
  optional match (b:Turf)-[:ASSIGNED]->(:Team)-[:MEMBERS]->(a)
    with a, collect(b.id) as tt
  optional match (b:Turf)-[:ASSIGNED]->(a)
    with tt + collect(b.id) as turfIds
  match (b:Turf) where b.id in turfIds
  ...
  optional match (person:Person)-[:RESIDENCE]->(unit:Unit)-[:AT]->(a)
    with a, collect(person{.*, unit:unit{.*}}) as upeople
  optional match (person:Person)-[:RESIDENCE]->(a)
    with a, upeople + collect(person{.*}) as people
  unwind people as person
  match (attr:Attribute)<-[:ATTRIBUTE_TYPE]-(pattr:PersonAttribute)-[:ATTRIBUTE_OF]->(fp:Person {id:person.id})
`+(req.query.formId?'where not (fp)<-[:INTERACTION_PERSON]-(:Interaction)-[:INTERACTION_FORM]->(:Form {id:{formId}})':'')+`
    with a, person, collect({id:attr.id, name:attr.name,value:pattr.value}) as attrs
    with a, a.position as ap, collect({person: person, attrs: attrs}) as people
  return collect({address: a{longitude:ap.x,latitude:ap.y,.street,.city,.state,.zip,.status,.updated}, people: people}) as data
    `, req.query);
  } catch (e) {
    return _500(res, e);
  }

  if (ref.data.length) return res.json(ref.data[0]);
  else return res.json([]);
}

async function dataAdd(req, res) {
  ...
  try {
    // for each new record ... create interaction
...
export function doExpressStartup() {
  ...
  app.get(base_uri+'/attribute/get', attributeGet);
  ...
  app.get(base_uri+'/data/get', dataGet);
  app.post(base_uri+'/data/add', dataAdd);
