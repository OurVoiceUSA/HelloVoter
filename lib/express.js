...
async function dataAdd(req, res) {
  ...
  if (isNaN(req.body.status) || [0,1,2].indexOf(req.body.status) === -1) return _400(res, "Invalid value to parameter 'status'.");
  ...
  // person required if there's data, and status is "home"
  if (req.body.status === 1 && !req.body.personId) return _400(res, "Invalid value to parameter 'personId'.");
