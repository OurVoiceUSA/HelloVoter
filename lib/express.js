...
async function peopleVisitUpdate(req, res) {
  ...
  `+(req.body.personId?'match (p:Person {id:{personId}})-[r:RESIDENCE {current:true}]->':'')+(req.body.unit?'(u:Unit {name:{unit}})-[:AT]->':'')+`(a:Address {id:{addressId}})-[:WITHIN]->(t)
...
`+(req.body.status===3?`
    set r.current = null, r.updated = timestamp()
`:`
    with vi, p
