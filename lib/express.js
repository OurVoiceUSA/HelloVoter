...
async function addressAddLocation(req, res) {
  ...
  // TODO: make sure this address is in a turf this volunteer is a leader of

  // TODO: check if address exists so we won't try to recreate it

  // create the visit + address
  await cqa(`match (v:Volunteer {id:{id}})<-[:USED_BY]-(d:Device {UniqueID:{deviceId}}), (f:Form {id:{formId}})
  create (a:Address {id:apoc.util.md5([toLower({street}), toLower({city}), toLower({state}), substring({zip},0,5)]), created: timestamp(), updated: timestamp(), position: point({longitude: toFloat({longitude}), latitude: toFloat({latitude})}), street: {street}, city: {city}, state: {state}, zip: {zip}})
  create (vi:Visit {
    start: {timestamp},
    end: {timestamp},
    status: 3,
  ...
  merge (vi)-[:VISIT_FORM]->(f)
    with a
  call spatial.intersects("turf", a.position) yield node
  merge (a)-[:WITHIN]->(node)`, req.body);

  // TODO: enqueue job to add address to "address" spatial plugin index; call spatial.addNode("address", a)
...
async function addressAddPerson(req, res) {
  if (!req.body.addressId) return _400(res, "Invalid value to parameter 'addressId'.");
...
async function visitsAndPeople(req, res) {
  ...
  try {
    ...
    if (!req.user.admin && req.user.autoturf) retry = 2;
...
`+((!req.user.admin&&req.query.formId)?'where (size(people) > 0 or size(units) > 0) and (not 2 in status or status is null)':'')+`
...
export function doExpressStartup() {
  ...
  app.post(base_uri+'/address/add/person', addressAddPerson);
