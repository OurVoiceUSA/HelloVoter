...
async function teamCreate(req, res) {
  ...
  try {
    ref = await db.query('match (a:Volunteer {id:{author_id}}) create (b:Team {id:randomUUID(), created: timestamp(), name:{name}})-[:CREATOR]->(a) return b.id', req.body);
  } catch (e) {
    return _500(res, e);
  }
...
async function turfCreate(req, res) {
  ...
  try {
    // create Turf
    ref = await db.query('match (a:Volunteer {id:{author_id}}) create (b:Turf {id:randomUUID(), created: timestamp(), name:{name}, geometry: {geometry}, wkt:{wkt}})-[:AUTHOR]->(a) WITH b, collect(b) AS t CALL spatial.addNodes(\'turf\', t) YIELD count return b.id', req.body);
  } catch(e) {
    return _500(res, e);
  }

  let job = await qq.queueTask('doTurfIndexing', 'Turf {id:{turfId}}', {turfId: ref.data[0]});
