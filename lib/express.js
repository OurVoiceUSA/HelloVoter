...
async function dataGet(req, res) {
  ...
  try {
    ...
    await cqa('match (a:Volunteer {id:{id}}) with a optional match (b:Turf)-[:ASSIGNED]->(:Team)-[:MEMBERS]->(a) with a, collect(b.id) as tt optional match (b:Turf)-[:ASSIGNED]->(a) with tt + collect(b.id) as turfIds match (b:Turf) where b.id in turfIds match (a:Address) using index a:Address(position) where distance(a.position, point({longitude: -74.38895, latitude: 41.71056})) < 1000 and (a)-[:WITHIN]->(b) with a, distance(a.position, point({longitude: -74.38895, latitude: 41.71056})) as dist order by dist limit {limit} optional match (person:Person)-[:RESIDENCE]->(unit:Unit)-[:AT]->(a) with a, collect(person) as unitpeople, unit optional match (person:Person)-[:RESIDENCE]->(a) with a, unitpeople + collect(person) as people, unit unwind people as person match (attr:Attribute)<-[:ATTRIBUTE_TYPE]-(pattr:PersonAttribute)-[:ATTRIBUTE_OF]->(person) with a, unit, {name:person.name, attrs:collect({name:attr.name,value:pattr.value})} as person return a as address, unit.name, collect(person) as people', req.query);
  } catch (e) {
    return _500(res, e);
  }
