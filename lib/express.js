...
async function volunteerAssignments(user) {
  ...
  try {
    ...
    if (ov_config.autoenroll_formid) {
      let b = await cqa('match (b:Form {id:{formId}}) return b{.*,direct:true}', {formId: ov_config.autoenroll_formid});
...
async function hello(req, res) {
  ...
  if (typeof req.body.dinfo === 'object') {
    try {
      ...
      await cqa('match (v:Volunteer {id:{id}}) merge (b:Device {UniqueID:{UniqueID}}) on create set b += {created: timestamp(), updated: timestamp(), '+dinfo_str+'} on match set b += {updated: timestamp(), '+dinfo_str+'} merge (v)<-[:USED_BY]-(b)', args);

      if (isNaN(args.lng) || isNaN(args.lat)) return _400(res, "Invalid value to parameters 'longitude' and 'latitude'.");

      await cqa('match (v:Volunteer {id:{id}}) set v.position = point({longitude: {lng}, latitude: {lat}})', args);
...
async function formGet(req, res) {
  ...
  try {
    ...
    form.attributes = [];
    let b = await cqa('match (a:Attribute)-[c:COMPILED_ON]->(b:Form {id:{formId}}) return a{.*, readonly: c.readonly}', req.query);
    for (let o in order) {
      let id = order[o];
      for (let i in b.data) {
...
async function addressAdd(req, res) {
  ...
  let ref = await cqa(`match (v:Volunteer {id:{id}})<-[:USED_BY]-(d:Device {UniqueID:{deviceId}}), (f:Form {id:{formId}})
  ...
  if (req.body.addressId && req.body.unit) {
    ...
    await queueTask('doAddAddress', 'Address {id:{addressId}}', req.body);
  }
...
*/
  ...
  // default limits; TODO: this is too large for dense urban areas
  if (!req.query.limit) req.query.limit = 10000;
  if (!req.query.dist) req.query.dist = 100000;
  ...
  `+((req.user.admin||req.user.autoturf)?'optional ':'')+`match (t:Turf) where t.id in turfIds
    with v, t, turfIds
`+(req.query.aId?`match (a:Address {id:{aId}})`:`match (a:Address) using index a:Address(position)`)+
((!req.user.admin||(req.user.admin&&!req.query.aId))?' where ':'')+
((req.user.admin?'':'((a)-[:WITHIN]->(t)'+(req.user.autoturf?' or distance(a.position, v.location) < {autoturf_dist}':'')+')'))+`
`+(req.query.aId?'':(req.user.admin?'':' and ')+`distance(a.position, point({longitude: {longitude}, latitude: {latitude}})) < {dist}
    with a, distance(a.position, point({longitude: {longitude}, latitude: {latitude}})) as dist
    order by dist limit {limit}`)+`
  optional match (u:Unit)-[:AT]->(a)
    with a, u
  optional match (person:Person)-[:RESIDENCE {current:true}]->(u)
  `+(req.query.filter_key?'where ((u)<-[:RESIDENCE {current:true}]-(:Person)<-[:ATTRIBUTE_OF {current:true}]-(:PersonAttribute {value:{filter_val}})-[:ATTRIBUTE_TYPE]->(:Attribute {id:{filter_key}}) or (person)<-[:ATTRIBUTE_OF {current:true}]-(:PersonAttribute {value:{filter_val}})-[:ATTRIBUTE_TYPE]->(:Attribute {id:{filter_key}}))':'')+`
`+((!req.user.admin&&req.query.formId)?(req.query.filter_key?'and':'where')+' not (person)<-[:VISIT_PERSON]-(:Visit)-[:VISIT_FORM]->(:Form {id:{formId}})':'')+`
...
`+((!empty_addrs&&req.query.formId)?'where size(people) > 0 or u is null':'')+`
`+(req.query.formId?'optional match (u)<-[:VISIT_AT]-(v:Visit)-[:VISIT_FORM]->(:Form {id:{formId}}) where v.status in {visit_status} with a, u, people, collect(v) as visits'+(req.user.admin?'':', collect(v.status) as status where not 2 in status or status is null'):'')+`
  ...
  `+(req.query.filter_key?'where ((a)<-[:RESIDENCE {current:true}]-(:Person)<-[:ATTRIBUTE_OF {current:true}]-(:PersonAttribute {value:{filter_val}})-[:ATTRIBUTE_TYPE]->(:Attribute {id:{filter_key}}) or (person)<-[:ATTRIBUTE_OF {current:true}]-(:PersonAttribute {value:{filter_val}})-[:ATTRIBUTE_TYPE]->(:Attribute {id:{filter_key}}))':'')+`
`+((!req.user.admin&&req.query.formId)?(req.query.filter_key?'and':'where')+' not (person)<-[:VISIT_PERSON]-(:Visit)-[:VISIT_FORM]->(:Form {id:{formId}})':'')+`
  optional match (attr:Attribute)<-[:ATTRIBUTE_TYPE]-(pattr:PersonAttribute)-[:ATTRIBUTE_OF {current:true}]->(person)
...
`+(req.query.formId?'optional match (a)<-[:VISIT_AT]-(v:Visit)-[:VISIT_FORM]->(:Form {id:{formId}}) where v.status in {visit_status} with a, units, ap, people, collect(v) as visits, collect(v.status) as status':'')+`
`+((!empty_addrs&&req.query.formId)?'where (size(people) > 0 or size(units) > 0) and (not 2 in status or status is null)':'')+`
...
async function peopleVisitUpdate(req, res) {
  ...
  try {
    ...
    if (req.addnewperson) {
      ...
      let ref = await cqa('match (p:Person {id:{personId}}) return count (p)', req.body);

      if (ref.data[0] > 0) return _403(res, "Person already exists.");

      await cqa('match (a:Address {id:{addressId}})'+(req.body.unit?'<-[:AT]-(u:Unit {name:{unit}})':'')+' create (p:Person {id:{personId}}) create (p)-[r:RESIDENCE {current:true}]->'+(req.body.unit?'(u)':'(a)'), req.body);
    }
