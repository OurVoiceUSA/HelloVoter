...
async function hello(req, res) {
  ...
  if (typeof req.body.dinfo === 'object') {
    try {
      ...
      await cqa('match (a:Volunteer {id:{id}}) merge (b:Device {UniqueID:{UniqueID}}) on create set b += {created: timestamp(), updated: timestamp(), '+dinfo_str+'} on match set b += {updated: timestamp(), '+dinfo_str+'} merge (a)<-[:USED_BY]-(b) set a.position = point({longitude: {lng}, latitude: {lat}})', args);
