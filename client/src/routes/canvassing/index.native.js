...
export default class Canvassing extends LocationComponent {
  ...
  constructor(props) {
    ...
    this.state = {
      refer: props.refer,
      server: 'gotv.ourvoiceusa.org',//props.server,
      orgId: 'UT9SJ7X',//props.orgId,
      admin: true,//props.navigation.state.params.admin,
      ...
      fUnit: {},
      canvassSettings: {},
      selectFormDialog: false,//(props.navigation.state.params.forms.length===1?false:true),
  ...
  _getCanvassSettings = async () => {
    ...
    if (form.default_filters && !canvassSettings.filters) {
      ...
      canvassSettings.filters = _.merge({}, form.default_filters);
    }
  ...
  _dataFetch = async (pos) => {
    ...
    try {
      ...
      if (!pressAddsSearchPin && !searchPins.length) {
  ...
  doRetry = async () => {
    let queue = _.merge({}, this.state.retry_queue);
